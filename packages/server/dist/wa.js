...
function createWhatsappSession(socket) {
    ...
    socket.on("create-wa-group", function (_a) {
        var groupName = _a.groupName, usersList = _a.usersList;
        console.log("create whatsapp group");
        waClient
            .createGroup(groupName, usersList.map(function (_a) {
            ...
            return waClient.sendMessage(res.gid._serialized, "Ciao raga, \uD83D\uDCAA\uD83C\uDFFB questo gruppo \u00E8 una prova ed \u00E8 stato creato da un bot! I partecipanti sono ".concat(usersList.map(function (user) { return user.fullName.split(" ")[0]; })));
        })
            .then(function (res) {
